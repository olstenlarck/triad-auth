...
function validAmbientConfig(keyring: string): Record<string, string> {
  return {
    ...
    ...requiredValues(keyring),
  };
}

function requiredValues(keyring: string): Record<string, string> {
  return {
...
describe("deployment configuration", () => {
  ...
  it("reports every missing variable without printing values", () => {
    ...
    try {
      ...
      expect(result.stderr.trim()).toBe(
        "Missing required configuration: SIGNING_KEYRING, IDENTIFIER_SECRET, CLAIMS_ENCRYPTION_KEYRING, RATE_LIMIT_SECRET\nAt least one complete provider credential pair is required",
      );
  ...
  it("rejects signing configuration without a complete provider pair", async () => {
    const directory = mkdtempSync(join(tmpdir(), "triad-config-"));
    const keyring = await generateSigningKeyring();
    const required = requiredValues(keyring);
    try {
  ...
  it("documents account deletion retention and deterministic resurrection", () => {
    ...
    expect(readme).toContain(
      "That continuity depends on preserving `IDENTIFIER_SECRET`; signing-key and claims-encryption-key rotation do not change these identifiers.",
    );
    expect(readme).toContain(
      "Global client registrations and rate-limit counters remain in place because they are not account data.",
    );
    expect(readme).toContain(
      "Device-client domain-verification cache entries also remain because they describe client control, not a user account.",
    );
    expect(readme).toContain(
  ...
  it("rejects malformed signing keyrings and short independent secrets without revealing them", () => {
    const directory = mkdtempSync(join(tmpdir(), "triad-config-"));
    try {
      writeFileSync(
        join(directory, ".dev.vars"),
  ...
  it.each([
    ...
    try {
      writeDevVars(directory, {
        ...requiredValues(await generateSigningKeyring()),
