...
async function seedGrant(
  ...
  if (status === "approved") {
    await env.DB.prepare(
      "INSERT OR IGNORE INTO accounts (id, created_at) VALUES ('acct_device', unixepoch())",
    ).run();
    await env.DB.prepare(
      `INSERT OR IGNORE INTO identities
      (provider, provider_user_id, account_id, created_at) VALUES ('github', '42', 'acct_device', unixepoch())`,
    ).run();
  }
  await env.DB.prepare(
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: string | URL | Request) => {
      ...
      if (url === "https://github.com/login/oauth/access_token") {
        return new Response(
          JSON.stringify({ access_token: "temporary", token_type: "bearer", scope: "" }),
          {
            status: 200,
...
function transitionAfterDeviceStateRead(
  env: Env,
  transition: (db: D1Database) => Promise<void>,
): void {
  const db = env.DB;
...
describe("device authorization", () => {
  it("issues RFC-shaped random codes and stores only the device-code hash", async () => {
    ...
    expect(new URL(body.verification_uri_complete).searchParams.get("user_code")).toBe(
      body.user_code,
    );

    const row = await env.DB.prepare(
      "SELECT device_code_hash, user_code FROM device_grants",
    ).first<{
      device_code_hash: string;
      user_code: string;
    }>();
    expect(row).toEqual({
      device_code_hash: await sha256(body.device_code),
      user_code: body.user_code.replace("-", ""),
    });
  ...
  it("requires a provider and rejects unavailable providers before creating a grant", async () => {
    ...
    expect(await env.DB.prepare("SELECT COUNT(*) AS count FROM device_grants").first("count")).toBe(
      0,
    );
  });
  ...
  it("rejects a non-origin or oversized issuance client", async () => {
    ...
    await env.DB.prepare(
      `INSERT INTO clients
      (client_id, name, redirect_uris, providers, created_at) VALUES ('blocked', 'Blocked', '[]', '[]', unixepoch())`,
    ).run();

    for (const clientId of ["unknown", "a".repeat(129), "blocked"]) {
      const response = await app.request(
        "/device/code",
        formRequest({ client_id: clientId, provider: "github" }),
        env,
      );
      expect(response.status).toBe(400);
      expect(response.headers.get("cache-control")).toBe("no-store");
  ...
  it("regenerates a colliding user code and inserts the next candidate", async () => {
    ...
    const response = await app.request(
      "/device/code",
      formRequest({ client_id: "triad-demo", provider: "github" }),
      env,
    );
    expect(response.status).toBe(200);
  ...
  it.each(["client_id", "provider", "scope"])(
    "rejects duplicate device issuance %s parameters",
    async (duplicate) => {
      const env = await testEnv();
      ...
      await expect(response.json()).resolves.toMatchObject({ error: "invalid_request" });
      expect(
        await env.DB.prepare("SELECT COUNT(*) AS count FROM device_grants").first("count"),
      ).toBe(0);
    },
  );
  ...
  it("enforces form encoding and the 4096-byte body limit on device POSTs", async () => {
    ...
    const wrongType = await app.request(
      "/device/code",
      { method: "POST", body: "client_id=triad-demo" },
      env,
    );
  ...
  it("normalizes inspected user codes and rotates a grant-bound CSRF token", async () => {
    ...
    const row = await env.DB.prepare(
      "SELECT purpose, COUNT(*) AS count FROM csrf_tokens GROUP BY purpose",
  ...
  it("carries encrypted minimal claims and canonical scopes through device exchange", async () => {
    ...
    const state = new URL(
      (await verified.json<{ redirect_to: string }>()).redirect_to,
    ...
    const stored = await env.DB.prepare(
      `SELECT scopes, claims_ciphertext FROM device_grants
    ...
    expect(
      await env.DB.prepare("SELECT COUNT(*) AS count FROM device_grants WHERE device_code_hash = ?")
        .bind(deviceHash)
        .first("count"),
    ).toBe(0);
  });
  ...
  it("rejects invalid or oversized user codes without issuing CSRF", async () => {
    ...
    expect(await env.DB.prepare("SELECT COUNT(*) AS count FROM csrf_tokens").first("count")).toBe(
      0,
    );
  });
  ...
  it("binds device confirmation callback to the browser that confirmed the code", async () => {
    ...
    const missingBrowser = await app.request(
      `/callback/github?state=${state}&code=provider-code`,
      undefined,
      env,
    );
  ...
  it("approves a pending grant only once across competing callbacks", async () => {
    ...
    for (const index of states.keys()) {
      await env.DB.prepare(
        ...
        .bind(stateHashes[index], deviceHash, await sha256(bindings[index]))
        .run();
    }
    stubGithub();
    ...
    const grant = await env.DB.prepare(
      "SELECT status, account_id, provider_sub FROM device_grants WHERE device_code_hash = ?",
    )
      .bind(deviceHash)
      .first();
  ...
  it("consumes a GitHub denial callback and makes the device poll return access_denied", async () => {
    ...
    await env.DB.prepare(
      ...
      .bind(stateHash, deviceHash, await sha256(binding))
      .run();
...
describe("device token exchange", () => {
  ...
  it("distinguishes fabricated and client-mismatched device codes from expired grants", async () => {
    ...
    await env.DB.prepare(
      ...
      VALUES ('other-client', 'Other', '[]', '["github"]', unixepoch())`,
    ).run();
    ...
    const mismatched = await app.request(
      "/token",
      deviceTokenRequest(deviceCode, "other-client"),
      env,
    );
  ...
  it("adds five seconds after an early poll", async () => {
    ...
    expect(
      await env.DB.prepare("SELECT interval_seconds FROM device_grants WHERE device_code_hash = ?")
        .bind(deviceHash)
  ...
  it("allows one concurrent first poll and slows the other", async () => {
    ...
    const errors = await Promise.all(
      responses.map((response) => response.json<{ error: string }>()),
    );
    expect(
      errors.map(({ error }) => error).sort((left, right) => left.localeCompare(right)),
    ).toEqual(["authorization_pending", "slow_down"]);
    expect(
      await env.DB.prepare("SELECT interval_seconds FROM device_grants WHERE device_code_hash = ?")
        .bind(deviceHash)
        .first("interval_seconds"),
  ...
  it("adds five seconds for every repeated early poll", async () => {
    ...
    for (const expectedInterval of [10, 15]) {
      ...
      expect(
        await env.DB.prepare(
          "SELECT interval_seconds FROM device_grants WHERE device_code_hash = ?",
        )
          .bind(deviceHash)
          .first("interval_seconds"),
  ...
  it.each(["approved", "denied"] as const)(
    ...
    async (status) => {
      ...
      transitionAfterDeviceStateRead(env, async (db) => {
        await db
          .prepare(
            `UPDATE device_grants SET status = ?, account_id = ?, provider_sub = ?
          WHERE device_code_hash = ?`,
          )
          .bind(
            status,
  ...
  it("atomically consumes an approved grant before issuing one token", async () => {
    ...
    expect(
      await env.DB.prepare(
        "SELECT COUNT(*) AS count FROM consents WHERE account_id = 'acct_device'",
      ).first("count"),
    ).toBe(1);
    expect(await env.DB.prepare("SELECT COUNT(*) AS count FROM device_grants").first("count")).toBe(
      0,
  ...
  it("deletes an approved grant before malformed claim ciphertext decryption", async () => {
    ...
    await env.DB.prepare(
      "UPDATE device_grants SET claims_ciphertext = 'v1.invalid' WHERE device_code_hash = ?",
    )
      .bind(deviceHash)
      .run();
    ...
    try {
      ...
      expect(
        await env.DB.prepare(
          "SELECT COUNT(*) AS count FROM device_grants WHERE device_code_hash = ?",
        )
          .bind(deviceHash)
          .first("count"),
      ).toBe(0);
  ...
  it("rejects token exchange after the client loses GitHub permission", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ error: "invalid_client" });
    expect(
      await env.DB.prepare("SELECT consumed_at FROM device_grants").first("consumed_at"),
    ).toBeNull();
  });
});
